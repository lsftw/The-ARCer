...
public final class Settings {
	...
	/**
	 * Helper of loadSettings(), verify type of settings' values, ex: boolean, int
	 */
	private static void verifySettings() {
		...
		verify("muted", VariableType.BOOLEAN, "false");
		verify("skipTitle", VariableType.BOOLEAN, "false");
	...
	private static void verify(String key, VariableType type, String defaultValue) {
		...
		if (value == null) {
			...
			case BOOLEAN:
				isValid = value.equalsIgnoreCase("true") || value.equalsIgnoreCase("false");
				break;
			case INTEGER:
				try {
			...
			default: // you goofed on an entry to verifySettings()
				isValid = false;
				Utility.printError("Programmer hardcoded error: Variable Type \"" + type + "\" not supported in verify(String,VariableType,String).");
				break;
			}
		}
		if (!isValid) {
	...
	protected static class SettingsFileReader extends SequentialFileReader {
		...
		protected void processLine(String curLine, int lineNumber) {
			...
			if (curLine.contains("=")) {
				...
				curValue = curLine.substring(curLine.indexOf('=') + 1).trim(); // Notice: case NOT discarded for value
